...
class create:
  def __init__(this, spotify):
    ...
    this.window.after(10, this.updateTime)
    this.window.bind('<Control-q>', this.quit)
  ...
  def createTitle(this, isPlaying):
    if(this.orientation == "horizontal"):
      ...
      this.playbackName = this.canvasTitle.create_text((int(25 * this.scale),int(50 * this.scale)),fill=this.rgbSecond,font=("Circular Bold", int(30 * this.scale)), text=this.songData.getPlaybackSource(this.orientation)[0], anchor='w', tags = 'playbackName')
      this.playbackArtist = this.canvasTitle.create_text((int(25 * this.scale),int(100 * this.scale)),fill=this.rgbThird,font=("Circular Bold", int(15 * this.scale)), text=this.songData.getPlaybackSource(this.orientation)[1], anchor='w')
      ...
      this.time.config(font=("Circular Bold", int(60 * this.scale)), bg=this.rgbMain, fg=this.rgbSecond)
      this.time.pack()
      ...
      this.playbackName = this.canvasTitle.create_text((int(this.screen_width/2),int(125 * this.scale)),fill=this.rgbSecond,font=("Circular Bold", int(20 * this.scale)), text=this.songData.getPlaybackSource(this.orientation)[0], anchor='center', tags = 'playbackName')
      this.playbackArtist = this.canvasTitle.create_text((int(this.screen_width/2),int(120 * this.scale)),fill=this.rgbThird,font=("Circular Bold", int(15 * this.scale)), text=this.songData.getPlaybackSource(this.orientation)[1], anchor='w')
      this.canvasTitle.tag_bind('playbackName','<Button-1>',this.playlistNameClick)
      this.canvasTitle.tag_bind('playbackName','<Enter>',lambda event, a='playbackName': this.mouseEnter(a))
      this.canvasTitle.tag_bind('playbackName','<Leave>',lambda event, a='playbackName': this.mouseLeave(a))
  ...
  def createTime(this, isPlaying):
    if(this.orientation == "horizontal"):
      ...
      this.songCurrentTime = this.canvasTimeBar.create_text((40 * this.scale, int(42 * this.scale)),fill=this.rgbSecond,font=("Circular Bold", int(13 * this.scale)), text=this.songData.getCurrentTime())
      this.songTotalTime = this.canvasTimeBar.create_text((int(660 * this.scale), int(42 * this.scale)),fill=this.rgbSecond,font=("Circular Bold", int(13 * this.scale)), text=this.songData.getTotalTime())
      ...
      this.songCurrentTime = this.canvasTimeBar.create_text((40*this.scale, int(42 * this.scale)),fill=this.rgbSecond,font=("Circular Bold", int(15 * this.scale)), text=this.songData.getCurrentTime())
      this.songTotalTime = this.canvasTimeBar.create_text((int(660 * this.scale), int(42 * this.scale)),fill=this.rgbSecond,font=("Circular Bold", int(15 * this.scale)), text=this.songData.getTotalTime())
  ...
  def createSong(this, isPlaying):
    if(this.orientation == "horizontal"):
      ...
      this.songTitle.config(font=("Circular Bold", int(26 * this.scale)), background= this.rgbMain, fg=this.rgbSecond)
      this.songTitle.pack()
      ...
      this.songArtist.config(font=("Circular Bold", int(15 * this.scale)),background= this.rgbMain, fg=this.rgbThird)
      this.songArtist.pack()
    else:
      this.songTitleText = StringVar()
      this.songTitleText.set(this.songData.name)
      this.songTitle = tk.Label(this.window, textvariable = this.songTitleText)
      this.songTitle.config(font=("Circular Bold", int(30 * this.scale)), background= this.rgbMain, fg=this.rgbSecond)
      this.songTitle.pack()

      this.songArtistText = StringVar()
      this.songArtistText.set(this.songData.artist)
      this.songArtist = tk.Label(this.window, textvariable = this.songArtistText)
      this.songArtist.config(font=("Circular Bold", int(20 * this.scale)),background= this.rgbMain, fg=this.rgbThird)
      this.songArtist.pack()
  ...
  def playlistNameClick(this, event):
    try:
      ...
      try:
        ...
        tk.Label(this.playMenuFrame, text='Playlists', justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 30)).pack()
        ...
        for playlist in this.playlistInfo:
          ...
          canvas.create_text((200,45),fill='white',font=("Circular Bold", 11), text = playlist['name'], anchor='w')
          canvas.create_text((500,45),fill='white',font=("Circular Bold", 11), text = str(playlist['songCount']) + ' songs', anchor='w')
          canvas.bind('<Button-1>',lambda event, a=playlist['id']: this.playlistClick(a))
          canvas.pack()
        height = len(this.playlistInfo) * 90 + 50
      except:
        tk.Label(this.playMenuFrame, text= 'No devices found!', justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 25)).pack()
        tk.Label(this.playMenuFrame, text= 'Open spotify on a device to load playlists.', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 16)).pack()
        height = 350
      ...
      try:
        devices = this.spotify.devices().get('devices')[0]['name']
        tk.Label(this.playMenuFrame, text='Playlists', justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 25)).pack()
        for playlist in this.playlistInfo:
          canvas = tk.Canvas(this.playMenuFrame, width = 600, bg = '#0f0f0f', height = 90, bd=0, highlightthickness=0)
          canvas.create_image((100,45), image=playlist['image'], anchor='center')
          canvas.create_text((200,45),fill='white',font=("Circular Bold", 11), text = playlist['name'], anchor='w')
        ...
        tk.Label(this.playMenuFrame, text= 'No devices found!', justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 25)).pack()
        tk.Label(this.playMenuFrame, text= 'Open spotify on a device to load playlists.', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).pack()
        height = 350
  ...
  def timeClick(this, event):
    try:
      ...
      tk.Label(this.controlMenu, text= 'Controls', justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 25)).pack()
      tk.Label(this.controlMenu, text= ' Click the time to refrence this window ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).pack()
      tk.Label(this.controlMenu, text = ' ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 6)).pack()
      tk.Label(this.controlMenu, text = ' NOTE: Playback Controls require a Spotify Premium Account', justify = 'center', bg = '#0f0f0f', fg = '#FF0000', font=("Circular Bold", 15)).pack()
      canvas1 = tk.Canvas(this.controlMenu, bg = '#0f0f0f', bd=0, highlightthickness=0)
      canvas1.pack()
      tk.Label(canvas1, text= ' Left Arrow ',  justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 0, column = 0, sticky = 'e')
      tk.Label(canvas1, text= ' Skip Song',      justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 0, column = 1, sticky = 'w')
      tk.Label(canvas1, text= ' Right Arrow ', justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 0, column = 2, sticky = 'e')
      tk.Label(canvas1, text= ' Previous Song ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 0, column = 3, sticky = 'w')

      tk.Label(canvas1, text= ' Up Arrow ',      justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 1, column = 0, sticky = 'e')
      tk.Label(canvas1, text= ' Increase Volume ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 1, column = 1, sticky = 'w')
      tk.Label(canvas1, text= ' Down Arrow ',    justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 1, column = 2, sticky = 'e')
      tk.Label(canvas1, text= ' Decrease Volume ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 1, column = 3, sticky = 'w')
 
      tk.Label(canvas1, text= ' S Key ',            justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 2, column = 0, sticky = 'e')
      tk.Label(canvas1, text= ' Toggle Shuffle ',     justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 2, column = 1, sticky = 'w')
      tk.Label(canvas1, text= ' R Key ',            justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 2, column = 2, sticky = 'e')
      tk.Label(canvas1, text= ' Toggle Repeat ',      justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 2, column = 3, sticky = 'w')

      tk.Label(canvas1, text= ' ? Key ',     justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 3, column = 0, sticky = 'e')
      tk.Label(canvas1, text= ' Open Control Menu ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 3, column = 1, sticky = 'w')
      tk.Label(canvas1, text= ' Space Bar ',     justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 3, column = 2, sticky = 'e')
      tk.Label(canvas1, text= ' Play/Pause ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 3, column = 3, sticky = 'w')
      
      tk.Label(canvas1, text= ' P Key ',                  justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 4, column = 0, sticky = 'e')
      tk.Label(canvas1, text= ' Open Playlist Menu ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 4, column = 1, sticky = 'w')
      tk.Label(canvas1, text= ' ESC ',                  justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).grid(row = 4, column = 2, sticky = 'e')
      tk.Label(canvas1, text= ' Close Prismatic Player ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).grid(row = 4, column = 3, sticky = 'w')
      tk.Label(this.controlMenu, text = ' ', justify = 'center', bg = '#0f0f0f', fg = 'white', font=("Circular Bold", 11)).pack()

      this.clearCacheButton = tk.Button(this.controlMenu, text = "Logout of your Account", command = this.clearCache, width = 20, font=("Circular Bold", 11), relief = 'flat', borderwidth=0, highlightthickness = 0, bd = 0)
      this.clearCacheButton.pack()
      tk.Label(this.controlMenu, text= ' Use this button to change accounts or reenter account info. ', justify = 'center', bg = '#0f0f0f', fg = '#1ed761', font=("Circular Bold", 11)).pack() 
